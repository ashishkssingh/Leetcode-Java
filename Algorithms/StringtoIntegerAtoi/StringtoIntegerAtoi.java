package Algorithms.StringtoIntegerAtoi;
 ...
 * Input: s = "42"
 * 
 * Output: 42 Explanation: The underlined characters are what is read in, the
 * caret is the current reader position. Step 1: "42" (no characters read
 * because there is no leading whitespace) ^ Step 2: "42" (no characters read
 * because there is neither a '-' nor '+') ^ Step 3: "42" ("42" is read in) ^
 * The parsed integer is 42. Since 42 is in the range [-231, 231 - 1], the final
 * result is 42. Example 2:
 * 
 * Input: s = " -42"
 * 
 * Output: -42 Explanation: Step 1: " -42" (leading whitespace is read and
 * ignored) ^ Step 2: " -42" ('-' is read, so the result should be negative) ^
 * Step 3: " -42" ("42" is read in) ^ The parsed integer is -42. Since -42 is in
 * the range [-231, 231 - 1], the final result is -42. Example 3:
 * 
 * Input: s = "4193 with words"
 * 
 * Output: 4193 Explanation: Step 1: "4193 with words" (no characters read
 * because there is no leading whitespace) ^ Step 2: "4193 with words" (no
 ...
 * 231 - 1], the final result is 4193. Example 4:
 * 
 ...
 * final result is 0. Example 5:
 * 
 * Input: s = "-91283472332"
 * 
 * Output: -2147483648 Explanation: Step 1: "-91283472332" (no characters read
 * because there is no leading whitespace) ^ Step 2: "-91283472332" ('-' is
 * read, so the result should be negative) ^ Step 3: "-91283472332"
 * ("91283472332" is read in) ^ The parsed integer is -91283472332. Since
 * -91283472332 is less than the lower bound of the range [-231, 231 - 1], the
 * final result is clamped to -231 = -2147483648.
 * 
 * 
 * Constraints:
 * 
 * 0 <= s.length <= 200 s consists of English letters (lower-case and
 * upper-case), digits (0-9), ' ', '+', '-', and '.'.
 */
...
class StringtoIntegerAtoi {
    public static int myAtoi(String s) {
        ...
        while (start < length && Character.isDigit(s.charAt(start))) {
            number = 10 * number + (s.charAt(start) - '0');
            if (isPos && number >= Integer.MAX_VALUE)
                return Integer.MAX_VALUE;
            if (!isPos && -number <= Integer.MIN_VALUE)
